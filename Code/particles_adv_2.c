...
double particle_lists_normal[NumberOfParticles][NumberOfParticles][3];
// a data structure to store the nearby particles for each particle in the normal direction
double particle_lists_wrap[NumberOfParticles][NumberOfParticles][3];
// a data structure to store the nearby particles for each particle in the wrap around direction
...
void updateBody(int NumberOfParticles, int current_timestep) {
  for(int i = 0; i<NumberOfParticles; i++){
    ...
    if (current_timestep % 100 == 0){
      // at a set periodicity
      for (int j = 0; j<NumberOfParticles; j++){
        // iterate through every other particle in the simulation
        if(i != j){
          ...
          if(distance < (2.5 * s)){
            ...
            particle_lists_normal[i][j][0] = normalX;
            particle_lists_normal[i][j][1] = normalY;
            particle_lists_normal[i][j][2] = normalZ;
            ...
            particle_lists_normal[i][j][0] = 0;
            particle_lists_normal[i][j][1] = 0;
            particle_lists_normal[i][j][2] = 0;
          }
          ...
          if (distance < (2.5 * s)){
            ...
            particle_lists_wrap[i][j][0] = otherX;
            particle_lists_wrap[i][j][1] = otherY;
            particle_lists_wrap[i][j][2] = otherZ;
            ...
            particle_lists_wrap[i][j][0] = 0;
            particle_lists_wrap[i][j][1] = 0;
            particle_lists_wrap[i][j][2] = 0;
          }
        }
      }
    }
    else{
      // iterate through all items in the list of neighbours
      for(int j = 0; j<NumberOfParticles; j++){
        // iterate through the number of particles
        double xDistance = particle_lists_normal[i][j][0];
        double yDistance = particle_lists_normal[i][j][1];
        double zDistance = particle_lists_normal[i][j][2];
        // save the distances

        double distance = sqrt(xDistance * xDistance + yDistance * yDistance + zDistance * zDistance);

        if (xDistance != 0 && yDistance != 0 && zDistance != 0){
          // if the distances are non-zero
          double euclidianForce = 4 * a * (((12 * pow(s,12)) / pow(distance,13)) - ((6 * pow(s,6))/pow(distance,7)));
          // calculate the force between he particles

          force[0] += (xDistance)/distance * euclidianForce;
          force[1] += (yDistance)/distance * euclidianForce;
          force[2] += (zDistance)/distance * euclidianForce;
          // calculate the force in each direction (x,y,z)
        }
      }
      for(int j = 0; j<NumberOfParticles; j++){
        // iterate through the number of particles
        double xDistance = particle_lists_wrap[i][j][0];
        double yDistance = particle_lists_wrap[i][j][1];
        double zDistance = particle_lists_wrap[i][j][2];
        // save the distances
